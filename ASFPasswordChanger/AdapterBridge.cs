            ...
            if (registerModule != null && adapterEndpoint != null)
            {
                ...
                if (result is string str)
                    if (str == pluginName)
                        ...
                        Utils.Logger.LogGenericWarning(str);
            }
        }
#if DEBUG
        catch (Exception ex)
        {
            Utils.Logger.LogGenericException(ex, "Community with ASFEnhance failed");
        }
#else
        catch (Exception)
        {
             Utils.Logger.LogGenericDebug("Community with ASFEnhance failed");
        }
#endif
        return false;
    }
